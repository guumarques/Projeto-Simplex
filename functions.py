# Bibliotecas
import pulp #resolve os cálculos do problema de programação linear
...
def create_simplex_table(num_vars, num_constraints, optimization_type):
    ...
    simplex_table = pd.DataFrame(columns=[f"x{i+1}" for i in range(num_vars)] + ["cond"] + ["LD"],
                                 index=["Função Objetivo"] + [f"Restrição {j+1}" for j in range(num_constraints)], dtype=float)
    ...
    if optimization_type == "Maximizar":
        ...
        simplex_table["cond"] = ["-"] + [">="] * num_constraints
    simplex_table["LD"] = 0
    simplex_table.iloc[:, :-2] = 0.0  # Preenche as variáveis de decisão com 0
...
def simplex_optimization(simplex_tableau, optimization_type):
    ...
    slacks = {name: constraint.slack for name, constraint in problem.constraints.items()}
     ...
     # Calcular deltas (lucros relativos)
    deltas = {}
    for var_name in variables:
        var = variables[var_name]
        reduced_cost = var.dj  # Custo reduzido diretamente do pulp
        deltas[var_name] = reduced_cost

    return solution, optimal_value, shadow_prices, slacks, deltas